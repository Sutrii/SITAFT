<div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-semibold text-[#2d3a32]">Kelola Data Pengguna</h2>
...
<div class="bg-white rounded-2xl shadow-sm p-5 mb-6">
    <div class="grid grid-cols-3 gap-4 items-end w-full">
        <div class="w-full">
            <label class="block text-sm font-medium text-[#2d3a32] mb-1">Nama Pengguna</label>
            <input type="text" id="filterNama" placeholder="Cari nama pengguna..."
                class="w-full border border-[#d8e4d8] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#3ea76a] focus:outline-none" />
        </div>

        <div class="w-full">
            <label class="block text-sm font-medium text-[#2d3a32] mb-1">Posisi</label>
...
<div class="bg-white rounded-2xl shadow-md p-6 overflow-x-auto">
    <table id="usersTable" class="display text-sm min-w-[950px]">
        <thead>
        ...
        <tbody>
            @forelse ($users as $index => $user)
                <tr>
                    ...
                    <td>{{ $user->email ?? '-' }}</td>
                    <td>{{ $user->password ? '••••••' : '-' }}</td>
                    <td>{{ $user->roleId == 1 ? 'Viewer' : 'Admin' }}</td>
                    <td>
                        @switch($user->positionId)
                            @case(1) Koordinator Tugas Akhir @break
                            @case(2) Dosen @break
                    ...
                    <td class="text-center">
                        <div class="flex justify-center gap-3">
                            ...
                            <button type="button" class="text-red-500 hover:text-red-700 transition"
                                onclick="confirmDelete('{{ route('users.destroy', $user->id) }}', '{{ $user->name }}')">
                                🗑️
                            </button>
                        </div>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="7" class="text-center text-gray-500 py-4">Tidak ada data pengguna</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="bg-white w-[400px] rounded-2xl shadow-lg overflow-hidden">
        ...
        <form action="{{ route('users.store') }}" method="POST" class="p-5 space-y-4">
            ...
            <div>
                ...
                <select name="positionId" id="positionSelectAdd" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-400"
                    onchange="toggleEmailPassword('add', this.value)">
                    <option value="">-- Pilih Posisi --</option>
                    <option value="1">Koordinator Tugas Akhir</option>
...
<div id="editModal"
    ...
    <div class="bg-white w-[400px] rounded-2xl shadow-lg overflow-hidden">
        ...
        <form id="editForm" method="POST" class="p-5 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Role</label>
                <select name="roleId" id="editRole" required
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Posisi</label>
                <select name="positionId" id="editPosition" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-400"
                    onchange="toggleEmailPassword('edit', this.value)">
                    <option value="1">Koordinator Tugas Akhir</option>
                    <option value="2">Dosen</option>
                    <option value="3">Mahasiswa</option>
                </select>
...
<style>
#usersTable tbody tr:nth-child(odd) { background-color: #fafdfa; }
#usersTable tbody tr:nth-child(even) { background-color: #ffffff; }
#usersTable tbody tr:hover { background-color: #f1f8f4; }
