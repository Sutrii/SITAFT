                    ...
                    <td class="text-center">
                        <div class="flex justify-center gap-3">
                            ...
                            <button class="text-red-500 hover:text-red-700 transition"
                                onclick="confirmDelete('{{ route('koordinator.skripsi.destroy', $skripsi->id) }}', '{{ $skripsi->judul_skripsi }}')">🗑️</button>
                        </div>
                    </td>
                </tr>
            @empty
...
<div id="addModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 backdrop-blur-sm">
    <div class="bg-white w-[400px] rounded-2xl shadow-lg overflow-hidden">
        ...
        <form action="{{ route('koordinator.skripsi.store') }}" method="POST" class="p-5 space-y-4">
            @csrf
            <div>
...
function openEditModal(id, nama, judul, bidang, dosen1, dosen2) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            editForm.action = `/dashboard/koordinator/skripsi/${id}`;
        }
    });
}
...
editForm?.addEventListener('submit', (e) => {
    ...
    Swal.fire({
        title: 'Menyimpan...',
        text: 'Mohon tunggu, sedang memperbarui data skripsi',
...
function confirmDelete(actionUrl, judul) {
    Swal.fire({
        title: `Hapus "${judul}"?`,
        text: "Data skripsi ini tidak bisa dikembalikan!",
        icon: 'warning',
        showCancelButton: true,
