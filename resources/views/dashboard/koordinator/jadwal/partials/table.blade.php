                    ...
                    @if(!(Auth::user()->roleId == 1 && Auth::user()->positionId == 3))
                        <div class="flex justify-center gap-3">
                            ...
                            <button type="button" class="text-red-500 hover:text-red-700 transition"
                                onclick="confirmDelete('{{ route('koordinator.jadwal.destroy', $jadwal->id) }}', '{{ $jadwal->mahasiswa->name ?? '-' }}')">
                                🗑️
                            </button>
                        </div>
                    @endif
...
<div id="addModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 backdrop-blur-sm">
    <div class="bg-white w-[480px] rounded-2xl shadow-lg overflow-hidden">
        <div class="bg-gradient-to-r from-green-500 to-green-700 text-white px-5 py-3 font-semibold text-lg">
            Tambah Jadwal
        </div>

        <form action="{{ route('koordinator.jadwal.store') }}" method="POST" class="p-5 space-y-4">
            @csrf
            <div>
                ...
                <select name="mahasiswaId" required class="w-full border border-gray-300 rounded-lg px-3 py-2">
                    ...
                    @foreach ($mahasiswas as $m)
                        <option value="{{ $m->id }}" data-jenis="{{ $m->jenis_seminar }}">{{ $m->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Sesi Ujian</label>
                    <select id="sesiSeminar" required disabled
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-gray-100 cursor-not-allowed text-sm">
                        <option value="">-- Pilih Mahasiswa Dulu --</option>
                    </select>
                    <input type="hidden" name="jam_seminar" id="jamSeminarHidden">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal (Wajib Sesuai Sesi)</label>
                    <input type="date" name="tanggal_seminar" id="tanggalSeminar" required disabled
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-gray-100 cursor-not-allowed focus:ring-2 focus:ring-green-400">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="status" id="addStatus" required class="w-full border border-gray-300 rounded-lg px-3 py-2">
                    <option value="Seminar Proposal">Seminar Proposal</option>
                    <option value="Seminar Hasil">Seminar Hasil</option>
                    <option value="Sidang Akhir">Sidang Akhir</option>
                </select>
    ...
    <div class="bg-white w-[400px] rounded-2xl shadow-lg overflow-hidden">
        ...
        <form id="editForm" method="POST" class="p-5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Sesi Ujian</label>
                    <select id="editSesiSeminar" required disabled
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-gray-100 cursor-not-allowed text-sm">
                        <option value="">-- Loading... --</option>
                    </select>
                    <input type="hidden" name="jam_seminar" id="editJamSeminarHidden">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal (Wajib Sesuai Sesi)</label>
                    <input type="date" name="tanggal_seminar" id="editTanggalSeminar" required disabled
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-gray-100 cursor-not-allowed focus:ring-2 focus:ring-blue-400">
                </div>
            </div>
...
<div id="importModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 backdrop-blur-sm">
    <div class="bg-white w-[400px] rounded-2xl shadow-lg overflow-hidden">
        ...
        <form action="{{ route('koordinator.jadwal.import') }}" method="POST" enctype="multipart/form-data" class="p-5 space-y-4">
            @csrf
...
$(document).on('change', 'select[name="skripsiId"]', function() {
    ...
    fetch(`/skripsi/${skripsiId}/detail`)
        ...
        .then(data => {
            ...
            const status = $(this).closest('form').find('select[name="status"]').val();
            const isStrict = status === 'Seminar Hasil' || status === 'Sidang Akhir';

            if (isStrict && data.penguji1.id && data.penguji2.id) {
                const select1 = $(this).closest('form').find('select[name="dosenId1"]');
                const select2 = $(this).closest('form').find('select[name="dosenId2"]');

                if (!select1.find(`option[value="${data.penguji1.id}"]`).length) {
                    select1.append(`<option value="${data.penguji1.id}">${data.penguji1.name}</option>`);
                }
                if (!select2.find(`option[value="${data.penguji2.id}"]`).length) {
                    select2.append(`<option value="${data.penguji2.id}">${data.penguji2.name}</option>`);
                }

                select1.val(data.penguji1.id);
                select2.val(data.penguji2.id);

                ['#pembimbing1', '#pembimbing2', 'select[name="dosenId1"]', 'select[name="dosenId2"]'].forEach(sel => {
                    const elm = $(this).closest('form').find(sel);
                    elm.css('box-shadow', '0 0 0 3px #bbf7d0');
                    setTimeout(() => elm.css('box-shadow', 'none'), 1200);
                });
            } else {
                $(this).closest('form').find('select[name="dosenId1"]').val('');
                $(this).closest('form').find('select[name="dosenId2"]').val('');
...
$(document).on('change', 'select[name="mahasiswaId"]', function() {
    ...
    if (jenis === 'seminar_proposal') jenis = 'Seminar Proposal';
    if (jenis === 'seminar_hasil') jenis = 'Seminar Hasil';
    if (jenis === 'sidang_akhir') jenis = 'Sidang Akhir';
    
    if (jenis) {
        $(this).closest('form').find('select[name="status"]').val(jenis);
        $(this).closest('form').find('select[name="status"]').css('box-shadow', '0 0 0 3px #bbf7d0');
        setTimeout(() => $(this).closest('form').find('select[name="status"]').css('box-shadow', 'none'), 1200);
    }
    ...
    fetch(`/jadwal/by-mahasiswa/${mhsId}`)
        ...
        .then(data => {
            ...
            // No longer forcing datetime since we use dynamic dropdowns!
        })
        .catch(err => console.error(err));
});
...
fetch(`/dosen/all`)
    ...
    let slotData = {}; // Store the results to map Sesi to penguji
    
    $(document).on('change', 'select[name="skripsiId"]', function () {
        const formId = $(this).closest('form').attr('id');
        const modalPrefix = formId === 'editForm' ? 'edit' : '';
        const skripsiId = $(this).val();

        const sesiSelect = document.getElementById(modalPrefix ? 'editSesiSeminar' : 'sesiSeminar');
        const tanggalInput = document.getElementById(modalPrefix ? 'editTanggalSeminar' : 'tanggalSeminar');

        if (!skripsiId) return;

        $(sesiSelect).empty().append('<option value="">-- Mencari Jadwal... --</option>');
        $(sesiSelect).prop('disabled', true).addClass('bg-gray-100 cursor-not-allowed');

        const status = $(this).closest('form').find('select[name="status"]').val() || '';
        const url = `/skripsi/${skripsiId}/available-slots?status=${encodeURIComponent(status)}`;
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.error) {
                    ...
                    $(sesiSelect).empty().append('<option value="">-- Kosong (Tidak ada overlap) --</option>');
                    return;
                }

                $(sesiSelect).empty().append('<option value="">-- Pilih Sesi Tersedia --</option>');
                
                slotData = {};
                let firstSlotKey = null;
                data.slots.forEach((slot, index) => {
                    const slotKey = `${slot.hari} ${slot.jam}`;
                    slotData[slotKey] = slot;
                    $(sesiSelect).append(`<option value="${slotKey}">${slot.hari}, ${slot.jam} (Penguji Siap)</option>`);
                    if (index === 0) firstSlotKey = slotKey;
                });

                $(sesiSelect).prop('disabled', false).removeClass('bg-gray-100 cursor-not-allowed');
                $(sesiSelect).css('box-shadow', '0 0 8px rgba(62, 167, 106, 0.6)');
                setTimeout(() => $(sesiSelect).css('box-shadow', 'none'), 1200);

                if (firstSlotKey) {
                    $(sesiSelect).val(firstSlotKey).trigger('change');
                }
            })
            .catch(err => {
                console.error('Error fetching slots:', err);
                $(sesiSelect).empty().append('<option value="">-- Gagal mencari slot --</option>');
            });
    });

    $(document).on('change', '#sesiSeminar, #editSesiSeminar', function () {
        const slotKey = $(this).val();
        const formId = $(this).closest('form').attr('id');
        const modalPrefix = formId === 'editForm' ? 'edit' : '';
        
        const tanggalInput = document.getElementById(modalPrefix ? 'editTanggalSeminar' : 'tanggalSeminar');
        const jamHidden = document.getElementById(modalPrefix ? 'editJamSeminarHidden' : 'jamSeminarHidden');

        if (!slotKey || !slotData[slotKey]) {
            $(tanggalInput).prop('disabled', true).addClass('bg-gray-100 cursor-not-allowed').val('');
            return;
        }

        const data = slotData[slotKey];
        jamHidden.value = data.jam;
        
        // Simpan hari yang diexpect untuk validasi nanti
        ...
        $(tanggalInput).css('box-shadow', '0 0 8px rgba(62, 167, 106, 0.6)');
        setTimeout(() => $(tanggalInput).css('box-shadow', 'none'), 1200);
    });

    $(document).on('change', '#tanggalSeminar, #editTanggalSeminar', function () {
        const expectedHari = $(this).data('expected-hari');
        if (!expectedHari || !$(this).val()) return;

        const date = new Date($(this).val());
        const dayNames = ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'];
        const actualHari = dayNames[date.getDay()];

        if (actualHari.toLowerCase() !== expectedHari.toLowerCase()) {
            Swal.fire('Hari Tidak Cocok', `Anda memilih hari ${actualHari}. Sesi ini hanya tersedia untuk hari ${expectedHari}!`, 'error');
            $(this).val('');
        }
    });

    $(document).on('change', 'select[name="status"]', function () {
        const skripsiSelect = $(this).closest('form').find('select[name="skripsiId"]');
        if(skripsiSelect.val()) {
            skripsiSelect.trigger('change');
        }
    });
...
function openEditModal(id, mahasiswaId, skripsiId, dosen1, dosen2, jadwalMulai, jadwalSelesai, status, ruang) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const d = new Date(jadwalMulai.split(' ')[0]);
            const dayNames = ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'];
            const actualHari = dayNames[d.getDay()];
            
            const jamMulai = jadwalMulai.split(' ')[1].substring(0,5).replace(':', '.');
            const jamSelesai = jadwalSelesai.split(' ')[1].substring(0,5).replace(':', '.');
            const currentJam = `${jamMulai} - ${jamSelesai}`;
            const slotKey = `${actualHari} ${currentJam}`;

            const sesiSelect = document.getElementById('editSesiSeminar');
            sesiSelect.innerHTML = `<option value="${slotKey}" selected>${actualHari}, ${currentJam} (Terjadwal)</option>`;
            sesiSelect.removeAttribute('disabled');
            sesiSelect.classList.remove('bg-gray-100', 'cursor-not-allowed');

            const hiddenJam = document.getElementById('editJamSeminarHidden');
            hiddenJam.value = currentJam;

            const tanggalInput = document.getElementById('editTanggalSeminar');
            tanggalInput.value = jadwalMulai.split(' ')[0];
            $(tanggalInput).data('expected-hari', actualHari);
            ...
            editForm.action = `/dashboard/koordinator/jadwal/${id}`;
